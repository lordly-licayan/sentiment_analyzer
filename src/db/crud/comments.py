...
def create_comments(db: Session, comments: list[schemas.CommentCreate]):
    db.bulk_insert_mappings(models.Comment, [comment.dict() for comment in comments])
    db.commit()


def list_all_comments(db: Session):
    return db.query(models.Comment).distinct(models.Comment.comment).all()


def list_last_comments(db: Session, limit: int = 100):
    return (
        db.query(models.Comment)
        .distinct(models.Comment.comment)
        .order_by(
...
def list_comments_by_file(db: Session, file_id: str):
    return db.query(models.Comment).filter(models.Comment.file_id == file_id).all()
...
def paginate_comments(
    ...
    if file_id:
        query = db.query(models.Comment).filter(models.Comment.file_id == file_id)
    else:
    ...
    comments = query.order_by(desc(models.Comment.id)).limit(limit).all()
